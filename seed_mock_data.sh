...
create_template() {
  ...
  echo "通知テンプレートを作成中 (Destination ID: ${dest_id})..." >&2
...
if ! created_destination_id=$(create_destination); then
  echo "Destinationの作成に失敗したため、Templateの作成を中止します。" >&2
  exit 1
fi
if [ -z "$created_destination_id" ]; then
  echo "Destination IDが取得できませんでした (空です)。Templateの作成を中止します。" >&2
  exit 1
fi

# 2. 取得したDestination IDを使ってテンプレートを作成
if ! create_template "${created_destination_id}"; then
